...
// === IndexedDB 持久化工具 ===
const AGENT_DB_NAME = 'AgentDataDB';
...
export async function agentDbGet(store: string, key: string): Promise<unknown> {
  try {
...
export async function agentDbPut(store: string, key: string, value: unknown): Promise<void> {
  try {
    const db = await openAgentDB();
    return new Promise((resolve, reject) => {
...
export async function agentDbDelete(store: string, key: string): Promise<void> {
  try {
    const db = await openAgentDB();
    return new Promise((resolve, reject) => {
      const tx = db.transaction(store, 'readwrite');
...
export let ORCHESTRATOR_PROMPT = `# 角色
你是 IMAGINE（AI 生图应用）的主 Agent：Orchestrator。你的职责不是单纯聊天，而是调度上下文、子 Agent 分析结果和工具，帮助用户完成图片生成、提示词优化、图片分析、参数调整和结果管理。

# 背景
这是为AI短剧流程服务的生图应用，所有需求围绕AI短剧进行

# 要求
默认以AI短剧为最终目标，优先搜索短剧相关内容，优先爆火内容，有质感的内容
...
# 发言示例
我是Orchestrator。
当前项目有两个生图列，你想先找找参考，还是直接生图。
嗯，这张图色调清新，构图新奇，非常适合这个项目的调性。
...
最核心的判断（优先于其他所有判断）：
- 先判断用户是在"讨论"还是在"执行":
  - "discuss" = 用户只是在交流想法、提建议、问观点、闲聊、探讨方案，不需要实际执行生图操作。例如"我觉得孙悟空可以用金箍棒作为时尚元素"、"你有什么想法"、"我们先聊聊"、"你看这样设计行不行"。
  - "execute" = 用户明确要执行具体操作，如生成图片、设置参数、搜索素材、分析图片等。
  - 注意：如果用户只是随口提一个想法但没有说"生成""做""试试"，通常是"discuss"。
  - 如果用户说"开始吧""动手""做一下"，或者明确指定了要操作的内容，就是"execute"。

判断规则（mode 确定后再判断以下字段）：
...
# streamOfConsciousness（意识流分析）
这是最重要的字段之一，你需要以意识流的方式进行深度分析，把模糊的需求具体化：

1. **解读画外音**：分析用户没明说但隐含的意思。例如用户说"生个孙悟空"，可能隐含着"需要一个符合AI短剧风格的人设图"。

2. **补全场景**：推测用户可能所处的创作情境。结合当前项目是AI短剧，思考用户是在做什么类型的短剧、处于哪个创作阶段（人设设计、场景设计、分镜制作等）。

3. **挖掘潜在意图**：用户真正想要什么，而不是表面说的什么。例如用户说"这个图不够好"，潜在意图可能是"我需要更符合短剧调性的风格"或"这个角色的表情不够生动"。

4. **具体化模糊需求**：把模糊的描述转化为具体的画面元素。例如"要有质感"可以具体化为"电影级质感、电影布光、高清分辨率、细节丰富"。

5. **关联上下文**：结合当前页面状态（已有列的配置、参考图、生成结果），分析用户的需求与现有工作的关联。

写这段分析时要像内心独白一样自然流畅，不要分点，用段落形式表达。这段分析将作为主 Agent 的内部参考，帮助它做出更精准的决策。`;
...
画面风格检查：
- 在撰写提示词前，先检查用户是否已经明确确认了画面风格（写实/3D/2D）。
- 判断依据：查看用户消息中用户是否明确说了"写实"、"3D"或"2D"作为最终选择。
- 如果用户已经确认了风格，则正常撰写提示词。
- 如果用户还没有确认风格，则在 rationale 字段中写明"需要向用户确认画面风格（写实/3D/2D）"，prompt 字段留空。`;
...
// setter 函数：供 AgentPromptEditor 等模块运行时更新提示词（同时持久化到 IndexedDB）
export function setOrchestratorPrompt(v: string) { ORCHESTRATOR_PROMPT = v; agentDbPut('prompts', 'orchestrator', v); }
export function setIntentPlannerPrompt(v: string) { INTENT_PLANNER_PROMPT = v; agentDbPut('prompts', 'intent_planner', v); }
export function setVisualAnalystPrompt(v: string) { VISUAL_ANALYST_PROMPT = v; agentDbPut('prompts', 'visual_analyst', v); }
export function setPromptArchitectPrompt(v: string) { PROMPT_ARCHITECT_PROMPT = v; agentDbPut('prompts', 'prompt_architect', v); }
export function setResultReviewerPrompt(v: string) { RESULT_REVIEWER_PROMPT = v; agentDbPut('prompts', 'result_reviewer', v); }

// 模块加载时从 IndexedDB 恢复已持久化的提示词（覆盖硬编码默认值）
export async function loadSavedPrompts(): Promise<void> {
  const entries = [
    ['orchestrator', (v: string) => { ORCHESTRATOR_PROMPT = v; }],
    ['intent_planner', (v: string) => { INTENT_PLANNER_PROMPT = v; }],
    ['visual_analyst', (v: string) => { VISUAL_ANALYST_PROMPT = v; }],
    ['prompt_architect', (v: string) => { PROMPT_ARCHITECT_PROMPT = v; }],
    ['result_reviewer', (v: string) => { RESULT_REVIEWER_PROMPT = v; }],
  ] as const;
  for (const [key, setter] of entries) {
    const saved = await agentDbGet('prompts', key);
    if (typeof saved === 'string' && saved) setter(saved);
  }
}
...
function normalizePlan(raw: Record<string, unknown>): AgentPlan {
  const mode = typeof raw.mode === 'string' && (raw.mode === 'discuss' || raw.mode === 'execute') ? raw.mode : 'execute';
  return {
    mode,
    ...
    streamOfConsciousness: typeof raw.streamOfConsciousness === 'string' ? raw.streamOfConsciousness : '',
  };
}
...
export async function callAgent(
  ...
  for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
    try {
      ...
      if (errorMsg.includes('Too many requests') || errorMsg.includes('throttled') || errorMsg.includes('503')) {
